...
const InputUI: React.FC<InputUIProps> = ({
  ...
  const validateInput = (value: string): string => {
    ...
    if (
      ...
      value.length < validationRules.minLength[0]
    )
      return validationRules?.minLength[1];
    if (
      validationRules?.maxLength &&
      value.length > validationRules.maxLength[0]
    )
      return validationRules?.maxLength[1];
    if (validationRules?.pattern && !validationRules.pattern[0].test(value))
      return validationRules?.pattern[1];
    return "";
  };

  const handleFocus = (event: React.FocusEvent<HTMLInputElement>) => {
    if (event.target.value === "") setPlaceholder("請輸入" + label);
  };
  ...
  return (
    <div className="inputContainer relative flex flex-col">
      ...
      />
      <span
        className={`inputLabel pointer-events-none absolute left-2 top-[6px] inline-block transform bg-white text-base font-light transition-all duration-300 ${errorMsg && "!text-red-500"}`}
      >
